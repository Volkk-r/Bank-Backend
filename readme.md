...
Вот **только изменения в формате Markdown**, которые нужно добавить в `README.md`:

---

### GET `/invoice-details`

Возвращает список детализированных данных по выставленным счетам.
...
Редактирует детализированную информацию по счёту по его `id`.

**Тело запроса:**

```json
{
  "description": "Новая информация по счёту",
  "status": "оплачен"
}
```
...
Удаляет детализированную информацию по счёту по его `id`.

**Пример запроса:**

```
DELETE /invoice-details/2
```
...
```json
{
  "message": "Детализация счёта удалена",
  "id": 2
}
```
